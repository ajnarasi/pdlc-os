...
export function buildPromptBundle(
  ...
  const systemPrompt = [
    ...
    `- archetypeId, when emitted, MUST be one of the canonical IDs from the bundled JTBD catalog: A1, A2, A3, B1, B2, B3, B4, D1, D2, D3, E1, E2, E3, F1, F2, H1, H2, H3, I1, I2, L1, L2, M1, M2, P1. Inventing a new archetype id will fail validation.`,
    `- All schema fields typed as string MUST be returned as strings, even when their value looks numeric. Examples: tier="1" (not 1), estimate="3d", priority="P0", target="≤8 seconds".`,
    `- All schema fields typed as array MUST be a real JSON array, never a stringified one.`,
    ``,
